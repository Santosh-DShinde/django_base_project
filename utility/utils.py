...
def get_cipher():
    """
    Returns a Fernet cipher instance using the ENCRYPTION_KEY from Django settings.
    """
    key = settings.ENCRYPTION_KEY  # Ensure this is a 32-byte key
    return Fernet(base64.urlsafe_b64encode(key.encode()))

def encrypt(data: str) -> str:
    """
    Encrypts the given string and returns a URL-safe encoded string.
    """
...
def decrypt(encrypted_data: str) -> str:
    """
    Decrypts the given encrypted string and returns the original data.
    """
    cipher_suite = get_cipher()
